...
class initDatabase {
    ...
    function init() {
	    ...
	    delete('MKTK_LS_TEST'); delete('MKTK_TEST_QS');
            ...
            reset_auto_increment('MKTK_OCCR'); reset_auto_increment('MKTK_LS_THEME');
            reset_auto_increment('MKTK_LS_FIELD'); reset_auto_increment('MKTK_LS_TEST');
	    reset_auto_increment('MKTK_LS_DATA'); reset_auto_increment('MKTK_LS');
            $r_flag = true;
            ...
            if(!$r_flag) { return 1; }
	    foreach ($format_data['Ls_Data'] as $var) { $r_flag = insert('MKTK_LS_DATA', ['LSID', 'CONTENT'], [$var[0], $var[1]]); if(!$r_flag) { break; } }
            if(!$r_flag) { return 1; }
	    foreach ($format_data['Ls_Test'] as $var) { $r_flag = insert('MKTK_LS_TEST', ['LSID', 'QUESTION', 'ANSWER', 'DIFFICULTY'], [$var[0], $var[1], $var[2], $var[3]]); if(!$r_flag) { break; } }
            if(!$r_flag) { return 1; }
	    $r_flag = update('MKTK_USERS_SET', 'COMPLETEDFLAG', '3', 'WHERE COMPLETEDFLAG <> 4');
            if(!$r_flag) { return 1; }
            return 0;
        }
    }
