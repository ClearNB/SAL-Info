...
if ($method === 'GET') {
    ...
    $userid = filter_input(INPUT_POST, 'ch_n_n_ui', FILTER_SANITIZE_STRING);
	...
	if (session_chk()) {
	    ...
	    switch ($function) {
		case 2: //CHANGE USERID
		    $sql = update('MKTK_USERS', 'USERID', "$userid");
		    if (!$sql) {
			$code = 1;
		    }
		    break;
		case 3: //CHANGE USERNAME
		    $sql = update('MKTK_USERS', 'USERNAME', "$username");
		    if (!$sql) {
			$code = 1;
		    }
		    break;
		case 4: //CHANGE PASSWORD
		    ...
		    $sql = update('MKTK_USERS', 'PASSWORDHASH, SALT', $hash . ', ' . $salt);
		    if (!$sql) {
			$code = 1;
		    }
		    break;
