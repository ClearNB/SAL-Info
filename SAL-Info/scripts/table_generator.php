...
function status_table() {
    ...
    $table_title = '研修状況一覧';
    $table_title_icon = 'book';
    $table_headers = [
        ["ユーザ名", "研修内容", "研修状況", "研修数", "達成研修数", "達成度", "最終更新日時"],
        ["USERNAME", "COUNTLS", "STATUS", "COUNTCPLS", "PERCOMPLETED", "LASTLESSONTIME"]
    ];
    $result = select(false, "MKTK_USERS", "");
    if ($result) {
        return table_horizonal($table_id, $table_title, $table_title_icon, $table_headers[0], $table_headers[1], $result);
    } else {
        return false;
    }
}
